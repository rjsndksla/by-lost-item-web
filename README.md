# 부용고 분실물 찾기 서비스

분실물 등록/조회, 이미지 업로드, 인증, 게시글 관리 등 다양한 기능을 제공하는 웹 서비스입니다.

## 주요 기능
- 분실물/습득물 게시글 등록 및 목록 조회
- 이미지 업로드 및 미리보기
- Supabase 인증(회원가입, 로그인, 이메일 인증)
- 게시글 상세 모달, 필터(분실/습득, 유형)
- 반응형 UI/UX, 모던 디자인
- 동아리 홍보 게시글 관리
- 관리자 기능 (게시글 삭제/수정)
- 모바일 최적화 네비게이션

## 기술 스택
- **Frontend**: HTML, CSS, JavaScript
- **Backend/DB/Storage/Auth**: Supabase
- **배포**: 정적 호스팅 (GitHub Pages, Vercel 등)

## 개발/실행 방법
1. 이 저장소를 클론합니다.
2. Supabase 프로젝트를 생성하고, 환경설정(`supabase-config.js`)을 본인 프로젝트에 맞게 수정합니다.
3. 로컬에서 테스트:
   - VSCode Live Server, Python http.server, 또는 Vercel CLI 등으로 실행
4. 배포: 정적 호스팅 서비스 사용

## 환경설정
- `supabase-config.js`에서 Supabase URL, ANON KEY를 본인 프로젝트에 맞게 입력해야 합니다.
- 이메일 인증, Storage 공개 설정 등은 Supabase 콘솔에서 관리합니다.

## 주요 페이지
- **index.html**: 메인 페이지 (분실물 검색)
- **buyong.html**: 부용관 메인 페이지
- **list.html**: 분실물 목록
- **register.html**: 분실물 등록
- **profile.html**: 사용자 프로필
- **club.html**: 동아리 홈
- **club-list.html**: 동아리 목록
- **club-register.html**: 동아리 등록
- **club_profile.html**: 동아리 프로필

## 폴더 구조
```
├── index.html
├── buyong.html
├── list.html
├── register.html
├── profile.html
├── club.html
├── club-list.html
├── club-register.html
├── club_profile.html
├── style.css
├── script.js
├── supabase-config.js
└── README.md
```

## 라이선스
MIT

---

문의/기여/이슈는 GitHub Issue 또는 PR로 남겨주세요. 
